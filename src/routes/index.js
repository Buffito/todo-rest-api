...
router.post('/auth', async (request, response) => {
    ...
    let authUser = await user.findOne({ email: authEmail });
    if (authUser) {
        ...
        if (hashedPassword) {
            ...
            renderCreateUserEJS(response, "Invalid password!");
        }
    } else {
        renderCreateUserEJS(response, "User not found!");
    }
});
...
router.post('/register', async (request, response) => {
    ...
    if (validator.validate(request.body.email)) {
        ...
        return renderCreateUserEJS(response, "Email was not valid!");
    }
    ...
    let authUser = await user.findOne({ email: registerEmail });
...
function renderCreateUserEJS(response, message) {
    response.render("create.ejs", { message: message });
}

function renderTasks(response, message) {
    toDoTask.find({ 'creator': loggedUser }, (err, tasks) => {
